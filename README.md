...
│   └── 📄 traefik.yml                   # Основной конфиг SSL (ОБНОВИТЕ EMAIL!)
├──
...
# Скачайте проект
cd /root
git clone YOUR_REPOSITORY_URL ai_project
cd ai_project
...
# ОБЯЗАТЕЛЬНО также обновите email в traefik.yml
...
**ВАЖНО: Также обновите email в traefik.yml:**
```bash
# Отредактируйте traefik/traefik.yml
nano traefik/traefik.yml

# Найдите строку:
email: svemail2@gmail.com
...
# 2. ВАЖНО: Проверить email в traefik.yml
nano traefik/traefik.yml
# Найти строку: email: svemail2@gmail.com
# Заменить на ваш email: email: your-email@example.com

# 3. Запустить проверку домена
chmod +x ssl-check.sh
./ssl-check.sh

# 4. Если проверка прошла, но сертификаты не получены
docker logs service_traefik | grep -i acme

# 5. Перезапустить Traefik
docker restart service_traefik

# 6. Убедиться что порт 80 доступен для challenge
